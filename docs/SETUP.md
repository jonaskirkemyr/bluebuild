...
| `setup-nix` fails with "Error saving receipt: read-only filesystem" | the installer picked its planner for *mutable* Linux and tried to `mkdir /nix` on the ostree root. It detects ostree by whether `ostree remote list` exits 0, which is not guaranteed here — [`nix.just`](../files/justfiles/nix.just) now passes `install ostree` explicitly. By hand: `sh ./install.sh install ostree` |
